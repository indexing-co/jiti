...
export const SVMTokenTransfers: SubTemplate = {
  ...
  transform(block) {
    ...
    for (const tx of (block.transactions as unknown[]) || []) {
      ...
      if (typeof process !== 'undefined' && process?.env?.IS_JITI_TEST === 'true') {
        ...
        if (knownTxsToSkip.includes(txHash)) {
          continue;
        }
        ...
        const knownTokensToSkip = [
          ...
          '4kbKgH7aax6wPXJn576N8eamcLKoxZ2WD1a5PF8s2jnA', // THOR
          '4zD1Bp7u6PYLEoCM6wR8fVNqj6aJ2P4QLiPD2dBWd5wy', // MUGA
          '2avgL3YP48N1rcFXwxHNW2whSp6VqQkaZcoTdZ2NLnof', // BAGS
          'C4Zhp86cKdBq1RobpvBePMQL5VFgjD4Nnzfrv1aRC5Vt', // GLOTTO
          'nNMAeUPpqUaUXuZ1jqbUjuB8XPokh4pJHFTxySz6FMr', // BILL
          'LLJNRnieVDXb3hz65MS3PWf3sZYDr3huVN2M7J6eMGF', // JSPR
        ];

        const actualBalanceDiffs: Record<string, bigint> = {};
        for (let i = 0; i < svmTx.meta.postBalances.length; i += 1) {
        ...
        for (const post of svmTx.meta.postTokenBalances) {
          const pre = svmTx.meta.preTokenBalances.find(
            (ptb) => ptb.mint === post.mint && ptb.owner === post.owner && ptb.accountIndex === post.accountIndex
          );
        ...
        for (const pre of svmTx.meta.preTokenBalances) {
          ...
          const post = svmTx.meta.postTokenBalances.find(
            (ptb) => ptb.mint === pre.mint && ptb.owner === pre.owner && ptb.accountIndex === pre.accountIndex
          );
          actualBalanceDiffs[key] = BigInt(post?.uiTokenAmount.amount || 0) - BigInt(pre.uiTokenAmount.amount);
        }

        const computedBalanceDiffs: Record<string, bigint> = {};
        for (const txfer of txTransfers) {
          ...
          computedBalanceDiffs[keyFrom] -= txfer.amount as bigint;
          computedBalanceDiffs[keyTo] += txfer.amount as bigint;
        }

        for (const key in computedBalanceDiffs) {
          if (['null', WSOL_MINT].includes(key.split('-')[0])) {
            continue;
          }
          ...
          if (computedBalanceDiffs[key] !== actualBalanceDiffs[key]) {
            ...
            console.log({
              ...
              matchingTxfers: txTransfers.filter(
                (m) => m.token === token && (m.from === account || m.to === account) && m.from !== m.to
              ),
            });
